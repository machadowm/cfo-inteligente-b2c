#!/usr/bin/env bash
# ==============================================================================
# CFO INTELIGENTE B2C - DEVOPS/SRE SHIELD: EXORCISTA DE BIND MOUNT TRAPS
# Script: exorcise-mount-trap.sh
# Nível: SRE / Arquitetura de Sistemas de Confiabilidade (Bank-Grade)
# Finalidade: Destruir pastas fantasmas no host, limpar volumes e injetar DDL
# ==============================================================================
...
log_info() {
    echo -e "$(date +"%Y-%m-%d %H:%M:%S") [${CYAN}INFO${NC}] - $1"
}

log_warn() {
    echo -e "$(date +"%Y-%m-%d %H:%M:%S") [${YELLOW}ALERTA${NC}] - $1"
}
...
log_info "Iniciando auditoria física e lógica do diretório: ${PROJECT_DIR}"

if [[ ! -d "$PROJECT_DIR" ]]; then
    log_error "Diretório do projeto não localizado em $PROJECT_DIR. Certifique-se de que a stack está clonada na pasta padrão."
    exit 1
fi

cd "$PROJECT_DIR"

# 1. Carregar segredos contidos no .env local se existirem
DB_USER="admin"
DB_NAME="cfo_b2c"
if [[ -f ".env" ]]; then
    log_info "Carregando variáveis de ambiente a partir do .env..."
    # Extrai sem quebrar por restrições de sub-shell ou caracteres especiais
    DB_USER=$(grep -E "^DB_USER=" .env | cut -d'=' -f2- || echo "admin")
    DB_NAME=$(grep -E "^DB_NAME=" .env | cut -d'=' -f2- || echo "cfo_b2c")
else
    log_warn "Arquivo .env não localizado. Usando fallbacks padrões de banco (User: admin, DB: cfo_b2c)."
fi

# 2. Desligar a infraestrutura e expurgar volumes fantasmas
log_info "Derrubando contêineres e limpando volumes do Docker Compose..."
docker compose down -v || {
    log_warn "O compose down tradicional falhou. Forçando desligamento agressivo..."
    docker ps -q --filter name=cfo_ | xargs -r docker kill
    docker compose down -v --remove-orphans
}

# 3. Investigar se existe BIND MOUNT TRAP (pastas fantasmas)
log_warn "Iniciando varredura profunda à procura de diretórios fantasmas no Host..."

# Checagem da armadilha na raiz
for file in "init.sql" "init-v6.sql"; do
    if [[ -d "./$file" ]]; then
        log_warn "ARMADILHA DETECTADA: O elemento './$file' é um DIRETÓRIO físico no Host!"
        log_info "Deletando pasta fantasma './$file'..."
        sudo rm -rf "./$file"
    elif [[ -f "./$file" ]]; then
        log_info "O elemento './$file' existe e é um arquivo de texto plano válido."
    else
        log_info "O elemento './$file' não existe na árvore (estado limpo)."
    fi
done

# Checagem na pasta de dados do PostgreSQL
if [[ -d "./postgres_data" ]]; then
    log_warn "Limpando diretório persistido antigo 'postgres_data' no Host para evitar Skipping Initialization..."
    sudo rm -rf ./postgres_data
fi

# 4. Escrever o SQL mestre (init-v6.sql completo) no host como arquivo plano de verdade
log_info "Injetando o Schema Master Consolidado (init-v6.sql) no sistema de arquivos do Host..."
...
log_info "Arquivo 'init-v6.sql' gerado fisicamente no Host com sucesso."

# 5. Criar redundância de arquivo físico para evitar falhas de nomenclatura
cp ./init-v6.sql ./init.sql
log_info "Arquivo de fallback 'init.sql' gerado com sucesso."

# 6. Reerguer a stack do Docker Compose do zero absoluto
log_info "Subindo a stack do Docker Compose..."
docker compose up -d --build

# 7. Aguardar o banco de dados reportar integridade saudável (service healthy)
log_info "Aguardando o PostgreSQL inicializar fisicamente e reportar integridade..."
COUNTER=0
while [[ $COUNTER -lt 15 ]]; do
    STATUS=$(docker inspect --format='{{json .State.Health.Status}}' cfo_postgres || echo '"unknown"')
    if [[ "$STATUS" == '"healthy"' ]]; then
        log_info "✅ O contêiner cfo_postgres reportou saúde ativa (healthy)!"
        break
    fi
    log_warn "Status atual de saúde do Postgres: ${STATUS}. Aguardando 2 segundos..."
    sleep 2
    COUNTER=$((COUNTER + 1))
done

if [[ $COUNTER -eq 15 ]]; then
    log_error "O PostgreSQL falhou ao atingir status saudável em tempo hábil. Verifique os logs com: docker logs cfo_postgres"
    exit 1
fi

# 8. Validação relacional interativa de catálogo final
log_info "Executando prova de fogo de catálogo de tabelas físicas..."
docker exec -it cfo_postgres psql -U "$DB_USER" -d "$DB_NAME" -c "\dt public.*" || {
    log_error "Erro relacional catastrófico: falha de autenticação ou de leitura no psql."
    exit 1
}

log_info "✅ EXORCISMO OPERACIONAL CONCLUÍDO COM SUCESSO!"
log_info "Todos os binds fantasma foram purgados e o banco foi completamente re-inicializado."
